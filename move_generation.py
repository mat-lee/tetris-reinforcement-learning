...
class MoveGenerator:
    ...
    def _check_add_to_sets(self, piece_type, piece_location, check_placement=False):
        ...
        if not self._already_checked(piece_type, piece_location):
            ...
            if check_placement:
                mino_coords = self.mino_coords_dict[self.piece.type][piece_location.rotation]
                offset = np.array([piece_location.x, piece_location.y + 1], dtype=np.int8)
                coords = mino_coords + offset

...
def gen_model_aux_l2(config) -> keras.Model:
    """
    Neural network that separates spatial board understanding from piece information.
    Auxiliary heads only predict spatial board properties (height, holes).
    """
    def ValueHead():
        def inside(x):
            ...
            x = keras.layers.BatchNormalization()(x)
            x = keras.layers.Activation('relu')(x)
            x = keras.layers.Dense(1, activation=('tanh' if config.use_tanh else 'sigmoid'), name="value", kernel_regularizer=keras.regularizers.l2(config.l2_reg))(x)
            return x
        return inside

    def PolicyHead():
        def inside(x):
            x = keras.layers.Dense(POLICY_SIZE, activation="softmax", name="policy", kernel_regularizer=keras.regularizers.l2(config.l2_reg))(x)
            return x
        return inside
    ...
    piece_dense1 = keras.layers.Dense(256)
    piece_dense2 = keras.layers.Dense(256)
    a_piece_processed = piece_dense1(a_pieces_flat)
    o_piece_processed = piece_dense1(o_pieces_flat)
    ...
    a_piece_processed = piece_dense2(a_piece_processed)
    o_piece_processed = piece_dense2(o_piece_processed)

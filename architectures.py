...
def test_8(config) -> keras.Model:
    # Uses l2 regularization instead of dropout
    def ValueHead():
        # Returns value; found at the end of the network
        def inside(x):
            x = keras.layers.Dense(config.value_head_neurons, 
                                kernel_regularizer=keras.regularizers.L2(config.l2_reg))(x)
            x = keras.layers.BatchNormalization()(x)
            x = keras.layers.Activation('relu')(x)
            x = keras.layers.Dense(1, activation=('tanh' if config.use_tanh else 'sigmoid'), 
                                kernel_regularizer=keras.regularizers.L2(config.l2_reg))(x)
    ...
    def PolicyHead():
        ...
        def inside(x):
            # Generate probability distribution
            x = keras.layers.Dense(POLICY_SIZE, activation="softmax", 
                                kernel_regularizer=keras.regularizers.L2(config.l2_reg))(x)
    ...
    # Take other features and use a dense layer to add channelwise
    dense_1 = keras.layers.Dense(config.filters, kernel_regularizer=keras.regularizers.L2(config.l2_reg))
    ...
    # 1x1 Kernel
    kernel_1 = keras.layers.Conv2D(config.kernels, (1, 1), kernel_regularizer=keras.regularizers.L2(config.l2_reg))
    a_grid = kernel_1(a_grid)
    o_grid = kernel_1(o_grid)
    ...
    # Shrink opponent grid info
    o_grid = keras.layers.Dense(config.o_side_neurons, kernel_regularizer=keras.regularizers.L2(config.l2_reg))(o_grid)
    o_grid = keras.layers.BatchNormalization()(o_grid)
    o_grid = keras.layers.Activation('relu')(o_grid)
...
### Postponed because I can make normal architecture better

def test_9(config) -> keras.Model:
    # Tests different architecture
    #
    # Take the opponent grid and features, and perform the same operations as normal
    # After the residual layers, some sort of value head feature, used in conjunction with active player features to bias active grid
    # Perform usual operations on active grid
    def ValueHead():
        # Returns value; found at the end of the network
        def inside(x):
            x = keras.layers.Dense(config.value_head_neurons)(x)
    ...
    def Trunk():
            # n layers: n - p residual, p global pooling
            # Evenly space blocks
            global_pool_indices = [round((i + 1) / (config.pooling_blocks + 1) * config.blocks) - 1 for i in range(config.pooling_blocks)]
...
def test_10(config):
    ...
    def ResidualLayer():
        ...
        def inside(x):
            ...
            y = keras.layers.Dropout(config.dropout)(y)
            y = keras.layers.Activation('relu')(y)
            y = keras.layers.Conv2D(config.filters, (3, 3), padding="same")(y)
    ...
    def GlobalPoolingLayer():
        ...
        def inside(x):
            ...
            # Use batch norm and relu on pooling layers
            pool_bias = keras.layers.BatchNormalization()(pool)
            pool_bias = keras.layers.Activation('relu')(pool_bias)

            # Global average and global pooling on the first cpool channels
            pool_avg = keras.layers.GlobalAveragePooling2D()(pool_bias)
            pool_max = keras.layers.GlobalMaxPooling2D()(pool_bias)

            pooled = keras.layers.Concatenate()([pool_avg, pool_max])

            # Fully connected layer
            pooled = keras.layers.Dense(config.filters - config.cpool)(pooled)

            # Reshape to match the remaining channels
            pooled = keras.layers.Reshape((1, 1, config.filters - config.cpool))(pooled)

            # Add the biases to the remaining channels
            rest = keras.layers.Add()([rest, pooled])

            # Concatenate the pool and biased channels
            y = keras.layers.Concatenate(axis=-1)([pool, rest])
